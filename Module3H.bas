...
ws.Range("K1").Value = "PercentChange"
    ...
    'locate the stock with the "Greatest % increase", "Greatest %
    ...
    ws.Range("Q4") = WorksheetFunction.max(ws.Range("I2:I" & lastrow))
    
    ' returns one less because header row not a factor.
    'match function will return the row index number in the column that matches the min and max values.
    ...
    volume_number = WorksheetFunction.Match(WorksheetFunction.max(ws.Range("I2:I" & lastrow)), ws.Range("I2:I" & lastrow), 0)
    
    ' final ticker symbol for  total, greatest % of increase and decrease, and average
    ' get the ticker symbol for the min and max values.
    
    ws.Range("P2") = ws.Cells(increase_number + 1, 8)
    ws.Range("P3") = ws.Cells(decrease_number + 1, 8)
    ws.Range("P4") = ws.Cells(volume_number + 1, 8)
    
    ws.Range("O2") = "Greatest % Increase"
    ws.Range("O3") = "Greatest % Decrease"
    ws.Range("O4") = "Greatest Total Volume"
    
    ws.Range("H1:O4").Columns.AutoFit
Next ws
